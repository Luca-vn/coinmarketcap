...
def log_cross_margin_data(filename: str = CROSSMARGIN_LOG_FILE):
    try:
        ...
        for asset in assets:
            ...
            if rate is not None:
                ...
                try:
                    ...
                    if len(df_asset) > 0:
                        last_rate = float(df_asset.iloc[-1]["hourly_rate"])
    ...
    for coin in assets:
        ...
        try:
            ...
            if not df_trade.empty:
                ...
                buy_vol = float(latest_trade.get("buy_volume", 0) or 0)
                sell_vol = float(latest_trade.get("sell_volume", 0) or 0)
        ...
        if ("Trap" in ba) or ("Trap" in so) or ("Tránh" in so):
            signal = "🚨 TRÁNH"
        elif (
            ("MUA" in ba)
            and (funding is not None) and (funding < -0.0003)
            and (cross is not None) and (cross > 0.00005)
            and ("Long" in so)
            and (buy_vs_sell_ratio is not None) and (buy_vs_sell_ratio > 1.2)
        ):
            signal = "💰 MUA mạnh"
        elif (
            ("BÁN" in ba)
            ...
            and (cross is not None) and (cross > 0.00005)
            and ("Short" in so)
            and (buy_vs_sell_ratio is not None) and (buy_vs_sell_ratio < 0.8)
        ):
            signal = "⚠️ BÁN mạnh"
        else:
            signal = "🤔 CHỜ"
